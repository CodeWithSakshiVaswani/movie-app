...
const MovieDetails = () => {
  ...
  return (
    <div className="flex items-center justify-between gap-8 px-[19px] py-[13px]">
      <img
      ...
      <div className="flex flex-col">
        ...
        <p className="flex mb-1.5 text-[8px]">
          {movie.genres?.map((genre) => (
            <div
              key={genre.id}
              className="border border-genreborder rounded-4xl py-0.5 px-1.5 mr-[5px] mb-[7px]"
            >
              {genre.name}
            </div>
          ))}
        </p>
        <p className="text-overviewtext mb-2. text-[8px]">{movie.overview}</p>
      </div>
      <div className="flex flex-col">
        <p className="text-[7px] py-1.5 px-[2.5px] mr-1 mb-1.5 border-t border-searchbar">
          <strong className="block text-[8px] text-blue font-semibold">
            Director
          </strong>
          {movie.directors?.join(",")}
        </p>
        <p className="text-[7px] py-1.5 px-[2.5px] mr-1 mb-1.5 border-t border-searchbar border-b">
          <strong className="block text-[8px] text-blue font-semibold">
            Actors
          </strong>
          {movie.cast?.join(",")}
